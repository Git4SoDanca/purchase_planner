-- View: worldship_pickingout_view

-- DROP VIEW worldship_pickingout_view;

CREATE OR REPLACE VIEW worldship_pickingout_view AS 
 SELECT stock_picking.name AS out_number,
    sale_order.name AS so_number_number,
        CASE
            WHEN sale_order.customer_payment_method = 20 THEN 1
            ELSE 0
        END AS cod_flag,
  ...
  WHERE (stock_picking.state::text = 'confirmed'::text OR stock_picking.state::text = 'assigned'::text) AND "left"(stock_picking.name::text, 3) = 'OUT'::text AND stock_picking.number_of_packages > 0
  ORDER BY stock_picking.name DESC;

ALTER TABLE worldship_pickingout_view
  OWNER TO sodanca;
COMMENT ON VIEW worldship_pickingout_view
  IS 'View for worldship Delivery Order processing';
